package imageselection_test

import (
	"path/filepath"
	"purser/internal/adapters/datastore"
	"purser/internal/adapters/store/imageselection"
	"purser/internal/ports"
	"purser/internal/ports/imageselectiontest"
	"testing"
...
func TestRepository_ImageSelectionRepositoryContract_Badger(t *testing.T) {
	imageselectiontest.TestImageSelectionRepository(t, func(t *testing.T) ports.ImageSelectionRepository {
		return newRepository(t, newBadgerDatastore(t))
	})
}

func TestRepository_ImageSelectionRepositoryContract_SQL(t *testing.T) {
	imageselectiontest.TestImageSelectionRepository(t, func(t *testing.T) ports.ImageSelectionRepository {
		return newRepository(t, newSQLDatastore(t))
	})
}

func newRepository(t *testing.T, ds datastore.Datastore) ports.ImageSelectionRepository {
	t.Helper()
	repo, err := imageselection.New("test", ds)
	if err != nil {
		t.Fatalf("imageselection.New returned error: %v", err)
	}
	return repo
}
