...
func TestRepository_PersonRepositoryContract_Badger(t *testing.T) {
	persontest.TestPersonRepository(t, func(t *testing.T) ports.PersonRepository {
		return newRepository(t, newBadgerDatastore(t))
	})
}

func TestRepository_PersonRepositoryContract_SQL(t *testing.T) {
	persontest.TestPersonRepository(t, func(t *testing.T) ports.PersonRepository {
		return newRepository(t, newSQLDatastore(t))
	})
}

func newRepository(t *testing.T, ds datastore.Datastore) ports.PersonRepository {
	t.Helper()
	repo, err := person.New("test", ds)
	if err != nil {
