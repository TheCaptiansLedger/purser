...
export function GenresPage() {
  ...
  const { data, isLoading } = useTags({ key: 'genre', contentType: 'movie' })
    ...
    <div>
      ...
      <div className="px-8 py-6">
        {isLoading ? (
          ...
          <EmptyState icon={Clapperboard} title="No genres yet" description="Tag movies with key=genre to see them here." accent={ACCENT} />
        ) : (
          <div className="flex flex-wrap gap-2">
            {filtered.map(tag => (
              <button
                key={tag.id}
                type="button"
                onClick={() => navigate(`/movies/genre/${toSlug(tag.value)}`)}
