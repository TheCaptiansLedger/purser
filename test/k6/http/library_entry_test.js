...
export default () => {
  ...
  res = http.post(`${SERVICE}/GetLibraryEntryDeletionImpact`, JSON.stringify({ id: id }), HEADERS);
  check(res, {
  ...
  res = http.post(`${SERVICE}/GetLibraryEntry`, JSON.stringify({ id: childId }), HEADERS);
  check(res, {
  ...
  res = http.post(`${SERVICE}/GetLibraryEntryDeletionImpact`, JSON.stringify({ id: childId }), HEADERS);
  check(res, {
    'GetLibraryEntryDeletionImpact reports the group as blocking': (r) =>
      (r.json('impacts') || []).some((i) => i.kind === 'group' && i.count === 1 && i.blocking),
  });

  res = http.post(`${SERVICE}/DeleteLibraryEntry`, JSON.stringify({ id: childId }), HEADERS);
  check(res, {
    'DeleteLibraryEntry without cascade is 400 (FailedPrecondition) when a Group exists': (r) => r.status === 400,
  });

  res = http.post(`${SERVICE}/DeleteLibraryEntry`, JSON.stringify({ id: childId, cascade: true }), HEADERS);
  ...
  res = http.post(`${GROUP_SERVICE}/GetGroup`, JSON.stringify({ id: groupId }), HEADERS);
