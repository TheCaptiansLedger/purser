...
export function MoviesPage() {
  ...
  return (
    <div>
      <PageHeader title="Movies" accent={ACCENT} search={search} onSearch={v => { setSearch(v); setOffset(0) }} total={data?.total} />

      <div className="px-8 py-6">
        {isLoading ? (
          <SkeletonGrid count={24} aspect="2/3" />
        ) : !data?.data.length ? (
          <EmptyState icon={Film} title="No movies yet" description="Add movies to your library to see them here." accent={ACCENT} />
        ) : (
          <>
