...
export default () => {
  ...
  res = http.post(`${PERFORMER_PROFILE}/CreatePerformerProfile`, JSON.stringify({ performerProfile: { personId: personId } }), HEADERS);
  ...
  res = http.post(`${BROWSE}/ListScenesInNetwork`, JSON.stringify({ networkId: networkId, pageSize: 10 }), HEADERS);
  check(res, {
  ...
  res = http.post(`${BROWSE}/ListScenesForPerformer`, JSON.stringify({ personId: personId, pageSize: 10 }), HEADERS);
  check(res, {
  ...
  res = http.post(`${BROWSE}/ListPerformersForScene`, JSON.stringify({ itemId: sceneId, pageSize: 10 }), HEADERS);
  check(res, {
  ...
  res = http.post(`${BROWSE}/ListPerformersForStudio`, JSON.stringify({ libraryEntryId: studioId, pageSize: 10 }), HEADERS);
  check(res, {
  ...
  res = http.post(`${BROWSE}/ListPerformersForNetwork`, JSON.stringify({ networkId: networkId, pageSize: 10 }), HEADERS);
  check(res, {
  ...
  res = http.post(`${BROWSE}/ListPerformers`, JSON.stringify({ pageSize: 10 }), HEADERS);
  check(res, {
  ...
  res = http.post(`${ITEM_PERSON}/DeleteItemPerson`, JSON.stringify({ itemId: sceneId, personId: personId, role: 'performer' }), HEADERS);
  check(res, { 'DeleteItemPerson status is 200': (r) => r.status === 200 });

  res = http.post(`${ITEM}/DeleteItem`, JSON.stringify({ id: sceneId }), HEADERS);
  check(res, { 'DeleteItem status is 200': (r) => r.status === 200 });

  res = http.post(`${PERFORMER_PROFILE}/DeletePerformerProfile`, JSON.stringify({ personId: personId }), HEADERS);
  check(res, { 'DeletePerformerProfile status is 200': (r) => r.status === 200 });

  res = http.post(`${PERSON}/DeletePerson`, JSON.stringify({ id: personId }), HEADERS);
  check(res, { 'DeletePerson status is 200': (r) => r.status === 200 });

  res = http.post(`${LIBRARY_ENTRY}/DeleteLibraryEntry`, JSON.stringify({ id: studioId }), HEADERS);
  check(res, { 'DeleteLibraryEntry(studio) status is 200': (r) => r.status === 200 });

  res = http.post(`${LIBRARY_ENTRY}/DeleteLibraryEntry`, JSON.stringify({ id: networkId }), HEADERS);
