...
// on PATH and an ACOUSTID_API_KEY env var (get one at
// https://acoustid.org/api-key); skips if either is missing. Run
// manually; computes a real fingerprint from the checked-in sample audio
// via the real Fingerprint, then calls the real Lookup with it — the
// sample is a synthetic tone, so no match is expected, but this proves
// the request/response shape this adapter assumes still holds against
// the real API.
...
func TestLive_LookupAgainstSampleFingerprint(t *testing.T) {
	...
	if apiKey == "" {
		t.Skip("ACOUSTID_API_KEY not set, skipping")
	}
