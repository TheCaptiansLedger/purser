...
func TestRepository_LibraryEntryRepositoryContract_Badger(t *testing.T) {
...
func TestRepository_LibraryEntryRepositoryContract_SQL(t *testing.T) {
	libraryentrytest.TestLibraryEntryRepository(t, func(t *testing.T) ports.LibraryEntryRepository {
		return newRepository(t, newSQLDatastore(t))
	})
}

func newRepository(t *testing.T, ds datastore.Datastore) ports.LibraryEntryRepository {
	t.Helper()
	repo, err := libraryentry.New("test", ds)
	if err != nil {
