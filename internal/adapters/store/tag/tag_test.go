...
func TestRepository_TagRepositoryContract_Badger(t *testing.T) {
	tagtest.TestTagRepository(t, func(t *testing.T) ports.TagRepository {
		return newRepository(t, newBadgerDatastore(t))
	})
}

func TestRepository_TagRepositoryContract_SQL(t *testing.T) {
	tagtest.TestTagRepository(t, func(t *testing.T) ports.TagRepository {
		return newRepository(t, newSQLDatastore(t))
	})
}

func newRepository(t *testing.T, ds datastore.Datastore) ports.TagRepository {
	t.Helper()
	repo, err := tag.New("test", ds)
	if err != nil {
