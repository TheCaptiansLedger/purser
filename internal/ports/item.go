...
// contentType, groupID, and status are independent, optional filters — an
// empty string (or domain.ItemStatus("") for status) means "no filter on
// this field."
type ItemRepository interface {
	...
	List(ctx context.Context, libraryEntryID, contentType, groupID string, status domain.ItemStatus, pageSize int, pageToken string) (items []*domain.Item, nextPageToken string, err error)
