...
func TestRepository_ItemRepositoryContract_Badger(t *testing.T) {
	itemtest.TestItemRepository(t, func(t *testing.T) ports.ItemRepository {
		return newRepository(t, newBadgerDatastore(t))
	})
}

func TestRepository_ItemRepositoryContract_SQL(t *testing.T) {
	itemtest.TestItemRepository(t, func(t *testing.T) ports.ItemRepository {
		return newRepository(t, newSQLDatastore(t))
	})
}

func newRepository(t *testing.T, ds datastore.Datastore) ports.ItemRepository {
	t.Helper()
	repo, err := item.New("test", ds)
	if err != nil {
