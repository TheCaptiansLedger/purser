...
import (
	...
	"purser/internal/adapters/store/image"
...
func TestRepository_ImageRepositoryContract_Badger(t *testing.T) {
	imagetest.TestImageRepository(t, func(t *testing.T) ports.ImageRepository {
		return newRepository(t, newBadgerDatastore(t))
	})
}

func TestRepository_ImageRepositoryContract_SQL(t *testing.T) {
	imagetest.TestImageRepository(t, func(t *testing.T) ports.ImageRepository {
		return newRepository(t, newSQLDatastore(t))
	})
}

func newRepository(t *testing.T, ds datastore.Datastore) ports.ImageRepository {
	t.Helper()
	repo, err := image.New("test", ds)
	if err != nil {
		t.Fatalf("image.New returned error: %v", err)
	}
	return repo
}
